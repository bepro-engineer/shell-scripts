...
#
# 戻り値　　　：0（正常終了）、1（警告）、2（異常終了）※JOB_OK, JOB_WR, JOB_ERで制御
# 使用箇所　　：システム間の中間ファイル転送、バッチ処理での安全なファイル受け渡し、統合システムやETL的用途でのデータ連携ステップ
#
# 設計書　　　：なし
#_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/
# ＜変更履歴＞
...
# 概要　　　：終了時のロック解除処理
# 説明　　　：
#   終了時にロックファイルを解除し、多重起動防止用の状態を後処理する。
#
# 引数　　　：なし
# 戻り値　　：なし
# 使用箇所　：trapによる終了時呼び出し
...
# 説明　　　：
#   指定されたファイルパスを基準に、転送状態を表す関連ファイルの存在有無を確認する。
#   判定結果として、INITIAL、LOADING、LOADED、COMPLETED のいずれかの状態文字列を返す。
#
# 引数　　　：$1 = 対象ファイルのパス
# 戻り値　　：状態文字列（INITIAL等）
# 使用箇所　：送信・受信処理判定時
...
# 説明　　　：
#   指定されたファイルとディレクトリの存在を確認し、引数の妥当性を検証する。
#
