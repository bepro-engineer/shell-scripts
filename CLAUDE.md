...
記載位置は shebang の直後とし、最低限下記を含めること。
...
- 必要に応じて `Command:` `Options:` `Example:` を追加する
- 実装に存在しない項目は勝手に追加しない

## Usage表示ルール

- Usage はヘッダコメントだけで終わらせない
- 引数なしで実行した場合は Usage を表示する
- 必須引数不足の場合は Usage を表示する
- 不正なオプション・不正な引数の場合は Usage を表示する
- `-h` または `--help` 指定時は Usage を表示する
- Usage の表示内容とヘッダコメントの記載内容は一致させる
- Usage は見本の形式に寄せて整形する
- 先頭と末尾に区切りラインを入れる
- `Usage:` 見出しを入れる
- 必要に応じて `Command:` `Options:` `Example:` を入れる
- 実装に存在しない項目は増やさない
- Usage 表示時の出力先は実装方針に従うが、少なくとも異常系で参照できる形にする
