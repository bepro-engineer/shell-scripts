...
- Bash 固有構文を使う実行スクリプトは `#!/bin/bash` を使用する
- POSIX 互換を前提とする実行スクリプトは `#!/bin/sh` を使用する
...
新規ファイル作成時は、ファイル先頭に用途が分かるコメントを付けること。
少なくとも、ファイル名、概要、役割、使用箇所が判別できる状態にすること。
既存ファイルにヘッダ形式がある場合は、その形式に合わせること。
